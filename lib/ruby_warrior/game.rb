...
#       end
        ...
        #MIRAR QUE EL DIRECTORIO EXISTA DE LO CONTRARIO REDIRECCIONAR AL INDEX DE WARRIORS CON MENSAJE DE ERROR.
        #EMPEZAR DE ACUERO AL TIPO DE DIFICULTAD QUE TENGA EL PLAYER SELECCIONADO
        
      profile = @player #ESTA QUEMADO, LA IDEA ES QUE SEA EL QUE SELECCIONÓ EL USUARIO
...
#       end
    ...
    def play_selected_level
      continue = true
      load_player
      
      game = current_level.play(@level)
    ...
    def load_player
      prefix = Rails.root.join('data_for_levels')
      load prefix.to_s+@player.data_for_level(@level.id).file_path
    end
    
    #ESTE METODO NO SE UTILIZARIA
    ...
    def play_epic_mode
      ...
      #TRAER LOS DATOS DEL OBJETO DE LA BDD
      #CARGAR EL NIVEL DE LA BDD
    ...
    def prepare_next_level
      #GUARDAR EN EL OBJETO DEL PERFIL DEL JUGADOR EL SIGUIENTE NIVEL ALCANZADO
    ...
    def prepare_epic_mode
      #CAMBIAR LA DIFICULTAD DEL PERFIL DEL JUGADOR
    ...
    # towers (REVISAR CREO QUE YA NO SE UTILIZARIAN)
    ...
    # levels (CARGAR DESDE LA BDD)
    
    def current_level
     #@current_level ||= profile.current_level
     Level.new(@player, @level.id)
    end
    ...
    #SE REPLAZARÍA POR EL INDEX DE WARRIORS DONDE SE ESCOJE EL WARRIOR
